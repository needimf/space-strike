...
const GameMessage = (props) => {
  ...
  if (!props.user.currentGame) {
    ...
    if (!(props.game.gameStatus === 'battle')) {
      ...
      instructions = <p className="center-align grey-text text-darken-3" style={{margin: '0.5rem 0 0'}}>Place your ships on the board</p>
    } else {
  ...
  return (
    <div id="message-bar" className="grey lighten-2">
      <div style={{margin: '0 0 0.5rem', padding: '1.9466666667rem 0.5rem 1rem'}}>
        <h3 id="message-header" className="center-align grey-text text-darken-3" style={{margin: '0 0 0.5rem'}} >{message}</h3>
        <div id="message-instructions" style={{margin: '0', padding: '0'}}>
          {instructions}
          {props.forfeitMsg && <p className="center-align grey-text text-darken-3" style={{margin: '0.5rem 0 0'}}>{props.forfeitMsg}</p>}
        </div>
      </div>
    </div>
  );
